...
<?php 
$useremail=$_SESSION['login'];
 $sql = "SELECT bikevehicles.Vimage1 as Vimage1,bikevehicles.VehiclesTitle,bikevehicles.id as vid,bikebrands.BrandName,bikebooking.FromDate,bikebooking.ToDate,bikebooking.message,bikebooking.Status,bikevehicles.PricePerDay,DATEDIFF(bikebooking.ToDate,bikebooking.FromDate) as totaldays,bikebooking.BookingNumber  from bikebooking join bikevehicles on bikebooking.VehicleId=bikevehicles.id join bikebrands on bikebrands.id=bikevehicles.VehiclesBrand where bikebooking.userEmail=:useremail order by bikebooking.id desc";
$query = $dbh -> prepare($sql);
$query-> bindParam(':useremail', $useremail, PDO::PARAM_STR);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
