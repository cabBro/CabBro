...
$sql="update bikevehicles set VehiclesTitle=:vehicletitle,VehiclesBrand=:brand,VehiclesOverview=:vehicleoverview,PricePerDay=:priceperday,FuelType=:fueltype,ModelYear=:modelyear,SeatingCapacity=:seatingcapacity where id=:id ";
...
<head>
	...
	<title>CabBro | Admin Edit Vehicle Info</title>
...
<?php 
$id=intval($_GET['id']);
$sql ="SELECT bikevehicles.*,bikebrands.BrandName,bikebrands.id as bid from bikevehicles join bikebrands on bikebrands.id=bikevehicles.VehiclesBrand where bikevehicles.id=:id";
$query = $dbh -> prepare($sql);
$query-> bindParam(':id', $id, PDO::PARAM_STR);
$query->execute();
...
<?php $ret="select id,BrandName from bikebrands";
